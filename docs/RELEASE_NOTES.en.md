...
- **Translate Disqus iframe comments** (#250)
  ...
  - The top frame forwards instructions to child iframes via `window.postMessage` — no extra permissions needed.
  - A custom signature, a same-frame guard, and an allow-list of actions block any other page from impersonating the extension.
...
- **Nudge you to pin the extension to your toolbar on install** (#244 / #247)
  ...
  - On browsers that can read the pin state (`chrome.action.getUserSettings()`, supported on Chrome 91+, Firefox 116+, etc.), the banner only shows when unpinned and disappears once you pin.
...
- **Fixed the translate bar showing up on pages with underscore-style lang codes like `<html lang="ja_JP">`** (#204)
  ...
  - Language-code comparison and display-name lookup now treat `_` the same as `-`, so `ja_JP` collapses to `ja` like it should.
...
- **Fixed the translate bar showing "This page is written in null"** (#195 / #197 / PR #198)
  ...
  - Padded values like `<html lang="en ">` and whitespace-only `<html lang="   ">` are now normalized (trim) so they don't misbehave.
