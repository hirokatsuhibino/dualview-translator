...
- **ページ全体翻訳が「見えている部分」から先に表示されるように**（#256）
  - これまでは DOM 順（ページ先頭から）に翻訳していたため、要素数の多い長いページでは画面に表示中の箇所の訳文が出るまで待たされることがあった
  - `IntersectionObserver` で画面内・画面近く（画面高の 1.5 倍先まで）の要素を優先的に翻訳し、画面外の要素はスクロールで近づいたタイミングで翻訳する方式に変更
  - 同時に翻訳される要素は「画面内 → 下方向 → 上方向」の順に並べ替え、読み進める方向を優先する
  - 画面外を翻訳しない分、翻訳 API の呼び出し回数も削減される
  - 未翻訳分が残る場合は完了トーストに「残り N 件はスクロール時」と表示して、翻訳漏れではないことを明示
  - 要素数が 30 未満のページ、翻訳＆要約モード、領域選択・右クリック翻訳・自動翻訳ルールは従来どおり全件即時翻訳
  - `IntersectionObserver` 非対応の環境では従来動作にフォールバック
  - 既知の制約: タブ UI の裏側など `display:none` の要素は、表示されるまで翻訳されない

### バグ修正

- DeepL 選択中に API キー未設定でページ翻訳を実行すると、対象要素に処理中マーク（`data-dvt-id="dvt-pending"`）が残り、API キー設定後も再翻訳できなくなる問題を修正（#256 対応中に発見）
...
- **Disqus iframe 内コメントの翻訳に対応**（#250）
  ...
  - トップフレームの content script が `window.postMessage` で配下 iframe に指示を伝播（追加 permission なし）
  - 受信メッセージを拡張独自シグネチャ + 許可 action リストに限定し、さらにトップフレームでは翻訳系アクションを実行しない（子 iframe のみ実行）ことで、外部ページからページ翻訳を操作されないようにしている
  - スコープ外: 選択テキスト翻訳・コンテキストメニュー翻訳・翻訳バー・自動翻訳ルール（OpenWeb / Facebook Comments 等の他コメントシステムも今回は非対応）
- **インストール時にツールバーへのピン留めを誘導**（#244 / #247）
  - 初回インストール直後にウェルカムページを開き、「パズルピース → ピンアイコン」の手順を案内
  - ポップアップ上部に誘導バナーを追加（閉じると次回以降は非表示）
  - ピン留め状態を判定できる環境（`chrome.action.getUserSettings()` 対応。Chrome 91+ / Firefox 116+ 等）では未ピン留め時のみ表示し、ピン留めすると自動で非表示
  - `getUserSettings` 非対応の古いブラウザでもバナーを表示するが、ポップアップを数回開くと自動的に表示を止める（永続的に居座らない）
  - ブラウザ仕様上ピン留めの強制はできないため、あくまで UX による誘導

### バグ修正

- **複数行選択時にミニアイコンがカーソルから離れた位置に表示されるバグを修正**（#240）
  - 複数行にまたがるテキストを選択した場合、`getBoundingClientRect()` が全行を囲む外接矩形を返すため、1 行目が長いと `right` がカーソル位置より大きくなりミニアイコンが右端寄りにずれていた
  - `getClientRects()` で行ごとの矩形を取得し、最終行の rect を使うよう変更
  - `getClientRects()` が空を返す環境（jsdom など）では従来の `getBoundingClientRect()` にフォールバック
...
- **選択時のミニアイコンを目立つデザインに変更**（#235）
  - サイズを 28px → 36px に拡大
  - 背景色を白から青（#1a73e8）に変更して視認性を向上
  - アイコンサイズも 16px → 20px に拡大
