...
- **設定のエクスポート / インポートを追加**（#206）
  ...
  - 翻訳先言語・エンジン選択・自動翻訳ルール・除外ドメイン・UI言語・テーマを JSON ファイルでバックアップ／復元できる
  - API キーは既定で含まれず、「APIキーを含める」チェックを ON にしたときだけ書き出される（漏洩リスクの明示的 opt-in）
  - インポートはエクスポートしたJSONを貼り付ける方式（Firefox popup ではファイル選択ダイアログ・alert・confirm を開くと popup が閉じてしまう既知の問題があるため、インライン状態表示で完結させる設計）
  - 翻訳・要約キャッシュおよびヒット率統計はエクスポート対象外（容量・端末固有のため）
