...
var DVT = (function () {
  ...
  // - Latin の `.!?` は、直後が空白または `[`（脚注番号開始）の場合に
  //   後続を確認して文末判定する。直後または空白後に Wikipedia 形式の
  //   脚注番号 `[数字]`（連続 `[10][11]` / 空白あり `[10] [11]` 含む）が
  //   あればスキップし、その先頭の非空白文字が大文字相当
  //   （A-Z / ひらがな / カタカナ / 漢字）の場合に分割。脚注番号は前文の末尾に取り込む。
  //   例: "sentence.[10] Next" → ["sentence.[10]", "Next"]
  //   例: "sentence. [10][11] Next" → ["sentence. [10][11]", "Next"]
